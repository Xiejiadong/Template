...
void move(data &x,data y)
{
    x.x=y.x;x.y=y.y;x.op=y.op;x.l=y.l;
}
...
void cdq(int l,int r)
{
    ...
    for (int i=l;i<=r;i++)
        if (i<=mid) q[i].l=1;else q[i].l=0; 
    int x=l,y=mid+1,s=0;
    while(1)//对第二维归并排序
    {
        if (x<=mid&&(y>r||q[x].x<=q[y].x)) move(temp[++s],q[x]),x++;
    ...
    {
        move(q[i],temp[++s]);
        if (temp[s].l==1) add(temp[s].y,1);
        else ans[temp[s].x]+=temp[s].op*sum(temp[s].y);
    }
    s=1;
    for (int i=l;i<=r;i++,s++)
        if (temp[s].l==1) add(temp[s].y,-1);
